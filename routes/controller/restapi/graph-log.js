 ...
 *
 * @METHOD get
 * @URL /restapi/v1/tso/graph-log
 * @PARAM start_date, end_date, floor_no, room_id
 */
router.get('/graph-log', function (req, res, next) {
	...
	pool.getConnection(function (err, connection) {
		...
		async.waterfall([
			async.apply(getGraphLogs, connection, query, result_data)
		], function (err) {
			connection.release();
			if (err) {
 ...
 *
 * @METHOD get
 * @URL /restapi/v1/tso/graph-all-log
 * @PARAM start_date, end_date
 */
router.get('/graph-all-log', function (req, res, next) {
	var query = utils.getRequestQuery(req);
	var new_data = req.body;
	var result_data = {};
	pool.getConnection(function (err, connection) {
		...
		async.waterfall([
			async.apply(getGraphAllLogs, connection, query, result_data)
		], function (err) {
			connection.release();
			if (err) {
				logger.error('==>> ERROR: %j', err);
...
function getGraphLogs(connection, query, result_data, callback) {
...
function getGraphAllLogs(connection, query, result_data, callback) {
    var sql = " select round(avg(temp),1) temp, updated_date, a.sensor_serial_no, room_name, floor_no from statistic_log a inner join metadata b on b.sensor_serial_no = a.sensor_serial_no ";
